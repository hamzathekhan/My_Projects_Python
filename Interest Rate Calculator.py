...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QCheckBox, QTreeView, \
    QVBoxLayout, QHBoxLayout, QLineEdit, QMainWindow, QMessageBox, QFileDialog
...
class FinanceApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Interest 2.0")
        self.resize(800,600)
        ...
        self.dark_mode.stateChanged.connect(self.toggle_style)

        self.apply_style()

    def apply_style(self):
        self.setStyleSheet('''
            FinanceApp { background-color : #f0f0f0 ;}
            QLabel , QLineEdit , QPushButton { background-color : #f8f8f8 ;}
            QTreeView { background-color : #ffffff ;}
        ''')

        if self.dark_mode.isChecked():
            self.setStyleSheet('''
                        FinanceApp { background-color : #222222 ;}
                        QLabel , QLineEdit , QPushButton , QCheckBox  { background-color : #333333 ; 
                                                            color : #eeeeee ;
                                                            }
                        QTreeView { background-color : #444444 ;
                                    color : #eeeeee ;
                                    }
                    ''')
    ...
    def save_data(self):
        ...
        if dir_path:
            ...
            QMessageBox.information(self,"Saved Data","Your Reulst is saved")
        else:
            QMessageBox.warning(self,"Path Not Found","Your Path is not Found")
